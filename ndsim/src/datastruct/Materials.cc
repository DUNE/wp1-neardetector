...
TGeoElement * Materials::FindElement(std::string name){
  ...
  if(!element_) std::cout << "\n***Element not found! " << name << std::endl;
...
TGeoMixture * Materials::FindMaterial(std::string name,bool deepCopy){
  ...
  if(!material_){
    std::cout << "\n***Material not found! " << name <<std::endl;
    return NULL;
  }
...
void Materials::DefineElements() {
  ...
  TGeoElement *elHydrogen 	= 	new TGeoElement("Hydrogen","Hydrogen",1,1.0791);
  TGeoElement *elBoron 		= 	new TGeoElement("Boron","Boron",5,10.811);
  TGeoElement *elCarbon 	= 	new TGeoElement("Carbon","Carbon",6,12.0107);
  TGeoElement *elNitrogen 	= 	new TGeoElement("Nitrogen","Nitrogen",7,14.0671);
  TGeoElement *elOxygen 	= 	new TGeoElement("Oxygen","Oxygen",8,15.99);
  TGeoElement *elFluorine 	= 	new TGeoElement("Fluorine","Fluorine",9,18.9984);
  TGeoElement *elSodium 	= 	new TGeoElement("Sodium","Sodium",11,22.99);
  TGeoElement *elMagnesium 	= 	new TGeoElement("Magnesium","Magnesium",12,24.305);
  TGeoElement *elAluminum 	= 	new TGeoElement("Aluminum","Aluminum",13,26.9815);
  TGeoElement *elSilicon 	= 	new TGeoElement("Silicon","Silicon",14,28.0855);
  TGeoElement *elPhosphorus 	= 	new TGeoElement("Phosphorus","Phosphorus",15,30.973);
  TGeoElement *elSulfur	        = 	new TGeoElement("Sulfur","Sulfur",16,32.065);
  TGeoElement *elArgon 		= 	new TGeoElement("Argon","Argon",18,39.948);
  TGeoElement *elPotassium	= 	new TGeoElement("Potassium","Potassium",19,39.0983);
  TGeoElement *elCalcium	= 	new TGeoElement("Calcium","Calcium",20,40.078);
  TGeoElement *elIron 		= 	new TGeoElement("Iron","Iron",26,55.845);
  TGeoElement *elCobalt		= 	new TGeoElement("Cobalt","Cobalt",27,58.93);
  TGeoElement *elCopper		= 	new TGeoElement("Copper","Copper",29,63.01);
  TGeoElement *elZinc		= 	new TGeoElement("Zinc","Zinc",30,65.38);
  TGeoElement *elLead		= 	new TGeoElement("Lead","Lead",82,207.2);
  ...
  elementList.push_back(elMagnesium);
  elementList.push_back(elAluminum);
  elementList.push_back(elSilicon);
  elementList.push_back(elPhosphorus);
  elementList.push_back(elArgon);
  elementList.push_back(elPotassium);
  elementList.push_back(elCalcium);
  elementList.push_back(elIron);
  elementList.push_back(elCobalt);
  elementList.push_back(elCopper);
  elementList.push_back(elLead);
  elementList.push_back(elZinc);
  elementList.push_back(elSulfur);
}
...
void Materials::DefineMaterials() {
  ...
  density = 0.0175*(CLHEP::g/CLHEP::cm3); // at 10 bar
  pressure    = 1*CLHEP::atmosphere;
  temperature = 293.15*CLHEP::kelvin;
  TGeoMixture * argonGas =  new TGeoMixture("T2KArgonGas",nel=4,density/(CLHEP::g/CLHEP::cm3));
  ...
  TGeoMixture *largon =  new TGeoMixture("LArgon",nel=1,density/(CLHEP::g/CLHEP::cm3));
  if(FindElement("Argon"))largon->AddElement(FindElement("Argon"), fractionmass = 1.000);
  if(largon){
    materialList.push_back(largon);
  }
  
  //Scintillator bars
  density = 1.050*(CLHEP::g/CLHEP::cm3); 
  TGeoMixture * scint =  new TGeoMixture("Scintillator",nel=2,density/(CLHEP::g/CLHEP::cm3));
  ...
  TGeoMixture * aluminum =  new TGeoMixture("Aluminum",nel=1,density/(CLHEP::g/CLHEP::cm3));
  if(FindElement("Aluminum"))aluminum->AddElement(FindElement("Aluminum"), fractionmass = 1.0);
  else aluminum = NULL;
  ...
  // Lead
  density = 11.34*(CLHEP::g/CLHEP::cm3);
  TGeoMixture * lead =  new TGeoMixture("Lead",nel=1,density/(CLHEP::g/CLHEP::cm3));
  if(FindElement("Lead"))lead->AddElement(FindElement("Lead"), fractionmass = 1.0);
  else lead = NULL;
  
  if(lead){
    materialList.push_back(lead);
  }
  ...
  density = 8.8*(CLHEP::g/CLHEP::cm3);
  TGeoMixture *brass = new TGeoMixture("Brass",nel=2,density/(CLHEP::g/CLHEP::cm3));
  if(FindElement("Copper"))brass->AddElement(FindElement("Copper"), fractionmass = 0.90);
  else brass = NULL;
  if(FindElement("Zinc"))brass->AddElement(FindElement("Zinc"), fractionmass = 0.10);
  else brass = NULL;
  if(brass){
  ...
  density = 1.7*(CLHEP::g/CLHEP::cm3);
  TGeoMixture* Dirt = new TGeoMixture("Dirt",nel=6,density/(CLHEP::g/CLHEP::cm3));
  if(FindElement("Oxygen"))Dirt->AddElement(FindElement("Oxygen"),fractionmass = 0.438);
  else Dirt = NULL;
  if(FindElement("Silicon"))Dirt->AddElement(FindElement("Silicon"),fractionmass = 0.257);
  else Dirt = NULL;
  if(FindElement("Sodium"))Dirt->AddElement(FindElement("Sodium"),fractionmass = 0.222);
  else Dirt = NULL;
  if(FindElement("Aluminum"))Dirt->AddElement(FindElement("Aluminum"), fractionmass = 0.049);
  else Dirt = NULL;
  if(FindElement("Iron"))Dirt->AddElement(FindElement("Iron"),fractionmass =  0.019);
  else Dirt = NULL;
  if(FindElement("Potassium"))Dirt->AddElement(FindElement("Potassium"),fractionmass =  0.015);
  else Dirt = NULL;
  ...
  // FeO
  density = 5.745*(CLHEP::g/CLHEP::cm3);
  TGeoMixture* FeO = new TGeoMixture("FeO",ncomponents=2,density/(CLHEP::g/CLHEP::cm3));
  if(FindElement("Iron"))FeO->AddElement(FindElement("Iron"),1);
  else FeO = NULL;
  if(FindElement("Oxygen"))FeO->AddElement(FindElement("Oxygen"),1);
  else FeO = NULL;

  if(FeO){
    materialList.push_back(FeO);
  }
  
  density = 3.97*(CLHEP::g/CLHEP::cm3);
  TGeoMixture* Al2O3 = new TGeoMixture("Al2O3",ncomponents=2,density/(CLHEP::g/CLHEP::cm3));
  if(FindElement("Aluminum"))Al2O3->AddElement(FindElement("Aluminum"),2);
  else Al2O3 = NULL;
  if(FindElement("Oxygen"))Al2O3->AddElement(FindElement("Oxygen"),3);
  else Al2O3 = NULL;

  if(Al2O3){
    materialList.push_back(Al2O3);
  }

  density = 3.58*(CLHEP::g/CLHEP::cm3);
  TGeoMixture* MgO = new TGeoMixture("MgO",ncomponents=2,density/(CLHEP::g/CLHEP::cm3));
  if(FindElement("Magnesium"))MgO->AddElement(FindElement("Magnesium"),1);
  else MgO = NULL;
  if(FindElement("Oxygen"))MgO->AddElement(FindElement("Oxygen"),1);
  else MgO = NULL;

  if(MgO){
    materialList.push_back(MgO);
  }

  density = 1.562*(CLHEP::g/CLHEP::cm3);
  TGeoMixture* CO2 = new TGeoMixture("CO2",ncomponents=2,density/(CLHEP::g/CLHEP::cm3));
  if(FindElement("Carbon"))CO2->AddElement(FindElement("Carbon"),1);
  else CO2 = NULL;
  if(FindElement("Oxygen"))CO2->AddElement(FindElement("Oxygen"),2);
  else CO2 = NULL;

  if(CO2){
    materialList.push_back(CO2);
  }

  density = 3.35*(CLHEP::g/CLHEP::cm3);
  TGeoMixture* CaO  = new TGeoMixture("CaO",ncomponents=2,density/(CLHEP::g/CLHEP::cm3));
  if(FindElement("Calcium"))CaO->AddElement(FindElement("Calcium"),1);
  else CaO = NULL;
  if(FindElement("Oxygen"))CaO->AddElement(FindElement("Oxygen"),1);
  else CaO = NULL;

  if(CaO){
    materialList.push_back(CaO);
  }

  density = 2.27*(CLHEP::g/CLHEP::cm3);
  TGeoMixture* Na2O  = new TGeoMixture("Na2O",ncomponents=2,density/(CLHEP::g/CLHEP::cm3));
  if(FindElement("Sodium"))Na2O->AddElement(FindElement("Sodium"),2);
  else Na2O = NULL;
  if(FindElement("Oxygen"))Na2O->AddElement(FindElement("Oxygen"),1);
  else Na2O = NULL;

  if(Na2O){
    materialList.push_back(Na2O);
  }

  density = 1.562*(CLHEP::g/CLHEP::cm3);
  TGeoMixture* P2O5  = new TGeoMixture("P2O5",ncomponents=2,density/(CLHEP::g/CLHEP::cm3));
  if(FindElement("Phosphorus"))P2O5->AddElement(FindElement("Phosphorus"),2);
  else P2O5 = NULL;
  if(FindElement("Oxygen"))P2O5->AddElement(FindElement("Oxygen"),5);
  else P2O5 = NULL;

  if(P2O5){
    materialList.push_back(P2O5);
  }

  //Rock
  density = 2.82*(CLHEP::g/CLHEP::cm3);
  TGeoMixture* Rock = new TGeoMixture("Rock",ncomponents=11,density/(CLHEP::g/CLHEP::cm3));
  if(FindMaterial("Sandstone",false))Rock->AddElement(FindMaterial("Sandstone",false), fractionmass = 0.5267);
  else Rock = NULL;
  if(FindMaterial("FeO",false))Rock->AddElement(FindMaterial("FeO",false), fractionmass = 0.1174);
  else Rock = NULL;
  if(FindMaterial("Al2O3",false))Rock->AddElement(FindMaterial("Al2O3",false), fractionmass = 0.1025);
  else Rock = NULL;
  if(FindElement("Oxygen"))Rock->AddElement(FindElement("Oxygen"),fractionmass = 0.0771);
  else Rock = NULL;
  if(FindMaterial("MgO",false))Rock->AddElement(FindMaterial("MgO",false), fractionmass = 0.0473);
  else Rock = NULL;
  if(FindMaterial("CO2",false))Rock->AddElement(FindMaterial("CO2",false), fractionmass = 0.0422);
  else Rock = NULL;
  if(FindMaterial("CaO",false))Rock->AddElement(FindMaterial("CaO",false), fractionmass = 0.0382);
  else Rock = NULL;
  if(FindElement("Carbon"))Rock->AddElement(FindElement("Carbon"),fractionmass = 0.024);
  else Rock = NULL;
  if(FindElement("Sulfur"))Rock->AddElement(FindElement("Sulfur"),fractionmass = 0.0186);
  else Rock = NULL;
  if(FindMaterial("Na2O",false))Rock->AddElement(FindMaterial("Na2O",false), fractionmass = 0.0053);
  else Rock = NULL;
  if(FindMaterial("P2O5",false))Rock->AddElement(FindMaterial("P2O5",false), fractionmass = 0.0007);
  else Rock = NULL;

  if(Rock){
    materialList.push_back(Rock);
  }
  /*
  // Material for the fiber core.
  density = 1.55*(CLHEP::g/CLHEP::cm3);
  TGeoMixture* CarbonFiber = new TGeoMixture("CarbonFiber",nel=3,density/(CLHEP::g/CLHEP::cm3));
  if(FindElement("Carbon"))CarbonFiber->AddElement(FindElement("Carbon"),fractionmass = 0.679);
  else CarbonFiber = NULL;
  if(FindElement("Hygrogen"))CarbonFiber->AddElement(FindElement("Hygrogen"),fractionmass = 0.057);
  else CarbonFiber = NULL;
  if(FindElement("Nitrogen"))CarbonFiber->AddElement(FindElement("Nitrogen"),fractionmass = 0.264);
  else CarbonFiber = NULL;

  if(CarbonFiber){
    materialList.push_back(CarbonFiber);
  }
  */
  //Earth
