...
SteppingAction::SteppingAction(G4double cutEnergy) 
  : rootGeoMgr_(NULL),eventID(0),cutEnergy_(cutEnergy){

  Initialise();
}

SteppingAction::SteppingAction(TGeoManager * geoMgr,G4double cutEnergy) 
  : rootGeoMgr_(geoMgr),eventID(0),cutEnergy_(cutEnergy){
...
SteppingAction::~SteppingAction()
{
  ...
  //delete allPartcilesVtr;

  if(rootGeoMgr_) delete rootGeoMgr_;
}
...
void SteppingAction::Initialise(){
  ...
  //primariesVtr    = new std::vector<GeantParticle>;
  
  //inParticleVtr   = new std::vector<GeantDaughterParticle>;
  //outParticleVtr  = new std::vector<GeantDaughterParticle>;
  //allPartcilesVtr = new std::vector<GeantDaughterParticle>;
  //allPartcilesVtr = new std::vector<GeantParticle>;
}
...
void SteppingAction::UserSteppingAction(const G4Step* theStep) { 
  ...
  */
  ...
  charge = theStep->GetTrack()->GetParticleDefinition()->GetPDGCharge();
  ...
  if( (theTrack->GetParentID()==0) && (particleSet(primariesVtr,trackID)==false) ){
    ...
    primariesVtr.push_back(particle);
  }
  else if(particleSet(allPartcilesVtr,trackID)==false){
    //GeantParticle prim;
    //this->setOriginalKinematics(prim,info);
 
    //GeantDaughterParticle particle;
    //this->setParticleKinematics(particle);
    ...
    //if(trackVolumeName.find("innerVessel") != std::string::npos){
      //std::cout << trackMotherVolumeName << " , " << pdg << " , " << magMom << " , trackID = " << trackID << std::endl;
      allPartcilesVtr.push_back(particle);
      //}
  }
  ...
  */
  ...
  if (steps>50000) {
    theTrack->SetTrackStatus(fStopAndKill);
    if(verbose_>0)
      std::cout <<"\n----Track Killed as too many steps: " << steps;
    steps = 0;
    return;
  }
...
bool SteppingAction::particleSet(std::vector<GeantParticle> particleVtr,int trackid){

  bool result = false;

  for(unsigned int i=0;i<particleVtr.size();i++){
    if(particleVtr.at(i).getTrackID() == trackid){
      result= true;
      break;
    }
  }
...
bool SteppingAction::findParent(GeantBasicParticle * p,TrackInformation * trackInfo){
  ...
  //loop over primaries 
  for(unsigned int i=0;i<primariesVtr->size();i++){
...
void SteppingAction::IncrementID(){
  eventID++;
  //if(particleEnterCount>0)
  //geantID++;
}

void SteppingAction::ClearTrackVtrs(){
  std::vector<GeantParticle>().swap(primariesVtr);
  primariesVtr.clear();
  primariesVtr.resize(0);
  //inParticleVtr->resize(0);
  //outParticleVtr->resize(0);
  std::vector<GeantParticle>().swap(allPartcilesVtr);
  allPartcilesVtr.clear();
  allPartcilesVtr.resize(0);
}
