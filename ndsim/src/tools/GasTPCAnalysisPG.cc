...
/*!
  ...
  \class    GasTPCAnalysisPG: This is in a development mode at the moment
...
int event_portion=2     // portion of processed event to report on screen...
  ;

TH1F *hNCharges, *hSigma;
TH2F *hSigVsZ;

TStopwatch fTimerAll, fTimerLoop;

void PrintTime(int neve) 
{
  fTimerAll.Stop(); fTimerLoop.Stop();
  double rtime = fTimerAll.RealTime(), ctime = fTimerAll.CpuTime(),
	 rtimel = fTimerLoop.RealTime(), ctimel = fTimerLoop.CpuTime();

  Info("PrintTime","%d events have been processed: "
		 "total CPU/RTU =%7.3f/%7.3f s: loop CPU/RTU =%7.3f/%7.3f s",
		 neve,ctime,rtime,ctimel,rtimel);
  cout.flush();

  fTimerLoop.Reset(); fTimerLoop.Start(); fTimerAll.Continue();
} // PrintTime

//_____________________________________________________________________________
Double_t  GetChargeDiffusionSigma(Double_t d) 
{
  // procedure returnes sigma of charge difussion = sqrt(2*D*t) = d*
  //  where: D - diffusion coefficient, t - collection time of charge,
  // created in the TPC at distance=d from acceleration plate,

  bool debug = true;
  debug = false;

  if (d<0)
	 Fatal("GetChargeDiffusionSigma","Negative charge travel distance=%g cm",d);
  
  double tm=d/fDriftVelocity;

  if (tm<0)
	 Fatal("GetChargeDiffusionSigma",
			 Form("Non-positive drift time=%g)",tm));
  
  double sig=sqrt(2*fDiffusionCF*tm);
  
  if (sig<0)
	 Fatal("GetChargeDiffusionSigma",
			 Form("Non-positive sigma spread on detection plate='%g')",fI0));
  
  if (debug)
	 Info("GetChargeDiffusionSigma","hit at dist=%g cm, travel time=%g msec, "
			"sigma on detection plate=%g cm",d,tm*1000,sig);
		
  return sig;

} // Matches Double_t  GetChargeDiffusionSigma(Double_t d)

//_____________________________________________________________________________
void  SimulateTPCDrift(TH2D *h2, Double_t edep_in_eV, Double_t hit_point[3])
{  
  // procedure emulates electron drifts in TPC from energy deposit edep_in_eV, expected in eV
  // created at point hit_point[3], expected in cm. 

  Bool_t debug=false;
  debug=true;

  if (debug) 
	 Info("SimulateTPCDrift","Diffuse energy deposit=%g keV @ X/Y/Z=%g/%g/%g "
			,edep_in_eV,hit_point[0],hit_point[1],hit_point[2]);
  
  Int_t NCharges = int(edep_in_eV/(fI0*fDiscFact));
  hNCharges->Fill(NCharges);

  if (debug)
	 Info("SimulateTPCDrift","Number of simulated charges=%d for energy deposit=%g keV"
			" and discrimination factor=%g",NCharges,edep_in_eV,fDiscFact);

  // distance from charge creation to side detector contact 
  // which is XY-plane with Z-location at -ACZN[2]
  Double_t d = hit_point[2] - fZ0;
  if (d<0) {
	 Warning("SimulateTPCDrift","hit Z=%g < TPC detection plate Z0=%g -> skip it!"
				,hit_point[2],fZ0);
	 return;
  }

  Double_t sig=GetChargeDiffusionSigma(d);
  hSigma->Fill(sig);
  hSigVsZ->Fill(hit_point[2],sig);

  for (int i = 0; i < NCharges; i++) {
	 Double_t dx,dy;       // x,y-shifts at TPC plate 
	                       // sampled from Gaussian with sigma from diffusion pars

	 dx = gRandom->Gaus(0,sig); dy = gRandom->Gaus(0,sig);

	 h2->Fill(hit_point[0]+dx,hit_point[1]+dy,fI0*fDiscFact);

	 if (debug && i<5)
		Info("SimulateTPCDrift","Original hit X/Y=%g/%g cm detected with shifts dX/dY=%g/%g cm"
			  ,hit_point[0],hit_point[1],dx,dy);
  }
  ...
  return;
} // Matches Bool_t  SimulateTPCDrift(TH2D *h2, Double_t edep_in_eV, Double_t hit_point[3])
...
int main(int argc, char ** argv) {
  ...
  fTimerAll.Reset(); fTimerAll.Start();
  fTimerLoop.Reset(); fTimerLoop.Start();
  ...
  memset (&HitPositionXY,0,sizeof(HitPositionXY));
  TString histname;
  Int_t bin  = 700; Int_t xmin  = -3500; Int_t xmax  = 3500;
  Int_t zmin=0, zmax=7000;
  if (savehisto) {
	 for(Int_t j = 0; j < n; j++){
		if (j==0) Info("GasTPCAnalysisPG::main",Form("%2d: Create Event Display histos",j));
		histname.Form("histoxy_%i",j);
		HitPositionXY[j] = new TH2D(histname.Data(),histname.Data(),bin,xmin,xmax,bin,xmin,xmax);
		HitPositionXY[j]->SetDirectory(0);
		histname.Form("histoxz_%i",j);
		HitPositionXZ[j] = new TH2D(histname.Data(),histname.Data(),bin,xmin,xmax,bin,zmin,zmax);
		HitPositionXZ[j]->SetDirectory(0);
		histname.Form("histoyz_%i",j);
		HitPositionYZ[j] = new TH2D(histname.Data(),histname.Data(),bin,xmin,xmax,bin,zmin,zmax);
		HitPositionYZ[j]->SetDirectory(0);

	 }
  }

  TH2D *hReadOutIdeal,*hReadOutReal;

  TString htitle;

  htitle=Form("Number of charges created per proton tracks; Number of charges; Counts");

  hNCharges = new TH1F("hNCharges",htitle,1024,0,2000);
  hNCharges->SetDirectory(0); hlist->Add(hNCharges);
  
  htitle=Form("Spread of charge at detection plate; Sigma, cm; Counts");
  hSigma = new TH1F("hSigma",htitle,1024,0,5);
  hSigma->SetDirectory(0); hlist->Add(hSigma);

  htitle=Form("Spread of detected charge vs. hit Z-coordinate; Hit Z, cm; Sigma, cm; Counts");
  hSigVsZ = new TH2F("hSigVsZ",htitle,256,0,500,256,0,2);
  ...
  // Event loop
  Info("GasTPCAnalysisPG::main","Number of processed/total events=%d/%d",n,nEntries);
  for(Int_t i=0; i<n; i++){

	 xmin=-1000; xmax=1000;
	 histname.Form("hReadOutIdeal_%03d",i);
	 htitle.Form("TPC ideal hit pattern in event=%d; X-axis, mm; Y-axis, mm",i);
	 hReadOutIdeal = new TH2D(histname,htitle,bin,xmin,xmax,bin,xmin,xmax);
	 hReadOutIdeal->SetDirectory(0); if(hlist) hlist->Add(hReadOutIdeal);
	 htitle.Form("TPC simulated real hit pattern in event=%d; X-axis, mm; Y-axis, mm",i);
	 histname.Form("hReadOutReal_%03d",i);
	 hReadOutReal = new TH2D(histname,htitle,bin,xmin,xmax,bin,xmin,xmax);
	 hReadOutReal->SetDirectory(0); if (hlist) hlist->Add(hReadOutReal);

	 //    if(i >= nentries) break;
	 
	 if ( (i%event_portion)==0 )
		PrintTime(i);
      ...
      for(Int_t j=0; j<tpcSdHits.size(); j++){
		  ...
		  if(value == tmpHit.getTrackID()){
			 ...
			 if(HitPositionXY[i]) {
				...
				hReadOutIdeal->Fill(tmpHit.getPosition().X(),tmpHit.getPosition().Y(),tmpHit.getEdep());
				Double_t hit_point[3]={tmpHit.getPosition().X()/10
											  ,tmpHit.getPosition().Y()/10
											  ,tmpHit.getPosition().Z()/10}
				,edep=tmpHit.getEdep();
								
				hname=hReadOutReal->GetName();
				Info("GasTPCAnalysisPG::main","ev%d, tpc hit=%d: before SimulateTPCDrift",i,j);

				SimulateTPCDrift(hReadOutReal,edep*1e9,hit_point); //
				
				Info("GasTPCAnalysisPG::main","ev%d, tpc hit=%d: after SimulateTPCDrift in %s",i,j,hname.Data());
				
				/* mock charge diffusion
				double sigm=20; int gain=10000;
				for (int k=0; k<gain; k++) {
				  hReadOutReal[i]->Fill(tmpHit.getPosition().X()+rand->Gaus(0,sigm)
												,tmpHit.getPosition().Y()+rand->Gaus(0,sigm)
												,tmpHit.getEdep()/gain);
				}
				*/
			 }
  ...
  for(Int_t j = 0; j < n; j++){
	 if (j==0) {
		if(HitPositionXY[j])
		  Info("GasTPCAnalysisPG::main",Form("%2d: Save Event Display histos",j));
		else {
		  Info("GasTPCAnalysisPG::main",Form("%2d: No Event Display histos",j));
		  break;
		}
	 }
	 ...
	 dhsn = hReadOutIdeal->GetName(); dhsn += Form("_%d",j);
	 dhst = hReadOutIdeal->GetTitle();

	 c1 = new TCanvas(dhsn.Data(),dhst.Data(),800,400);
	 c1->Divide(2,1);
	 c1->cd(1);
	 hReadOutIdeal->Draw("colz");
	 c1->cd(2);
	 hReadOutReal->Draw("colz");
	 c1->Write();
  }
...
void ReadDatacard(std::string datacard){
  ...
  fI0                  = util::XML2Dbl(xml,root_node,"DriftingModel/I0");

  if (fI0<=0)
	 Fatal("ReadDatacard",
			 Form("Non-positive TPC electric field=%g cm"
					"Set correct value='DriftingModel/I0' in config file!",fI0));
  Info("ReadDatacard","  Ionization potenial=%g eV",fI0);

  fDiffusionCF         = util::XML2Dbl(xml,root_node,"DriftingModel/DiffusionCF");

  if (fDiffusionCF<=0)
	 Fatal("ReadDatacard",
			 Form("Non-positive diffusion coefficient=%g cm"
					"Set correct value='DriftingModel/DiffusionCF' in config file!",fDiffusionCF));
  Info("ReadDatacard","  Working gas diffusion coefficient=%g cm2/sec",fDiffusionCF);

  fMobility            = util::XML2Dbl(xml,root_node,"DriftingModel/Mobility");

  if (fMobility<=0)
	 Fatal("ReadDatacard",
			 Form("Non-positive mobility coefficient=%g cm"
					"Set correct value='DriftingModel/Mobility' in config file!",fMobility));
  Info("ReadDatacard","  Working gas mobility coefficient=%g cm2/sec/V",fMobility);
  
  fEField              = util::XML2Dbl(xml,root_node,"DriftingModel/EField");
  
  if (fEField<=0)
	 Fatal("ReadDatacard",
			 Form("Non-positive TPC electric field=%g cm"
					"Set correct value='DriftingModel/EField' in config file!",fEField));
  Info("ReadDatacard","  TPC electric field=%g V/cm",fEField);

  fDriftVelocity=fMobility*fEField;
  Info("ReadDatacard","  Drift velosity=%g cm/msec",fDriftVelocity/1000);

  fZ0                  = util::XML2Dbl(xml,root_node,"DriftingModel/Z0");
  Info("ReadDatacard"," Z-coordinate of acceleration plate=%g cm",fZ0);

  fDiscFact            = util::XML2Dbl(xml,root_node,"DriftingModel/DiscFact");

  if (fDiscFact<=0)
	 Fatal("ReadDatacard",
			 Form("Non-positive TPC electric field=%g cm"
					"Set correct value='DriftingModel/DiscFact' in config file!",fDiscFact));
  Info("ReadDatacard","  Charge discrimination factor=%g",fDiscFact);

  // Release memory
  xml->FreeDoc(xmldoc);
  delete xml;
} // Matches void ReadDatacard(std::string datacard)
