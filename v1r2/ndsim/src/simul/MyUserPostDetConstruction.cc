...
MyUserPostDetConstruction::MyUserPostDetConstruction(std::string sdName,G4double magFieldStength) {
...
void MyUserPostDetConstruction::Initialize(TG4RootDetectorConstruction *det_){
  ...
  if ( SD_lv ) {
    ...
    std::cout << "\n*****************************************************"
		<< "\nMyUserPostDetConstruction::Found detector with name: " << SDName.c_str() 
		<< " -- SD set!" 
		<< "\n*****************************************************" <<std::endl;
  
    //Add the magnetic field 
    //magnetise the sensitive detector
    fpMagField_ = new MagneticField(G4ThreeVector(MagFieldStength*CLHEP::tesla,0,0),SD_lv);
      	...
      	std::cout << "\n*****************************************************"
			<< "\nMyUserPostDetConstruction::Cannot find detector with name: " << SDName.c_str() 
			<< " -- No SD set!" 		
			<< "\n*****************************************************" <<std::endl;
      	SD_lv = 0;
