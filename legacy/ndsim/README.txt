...
-------------------------------------------------------------------------------------------------------------------------------
*** The information below can be useful, howeve we have not validated the other executables yet *** 
-------------------------------------------------------------------------------------------------------------------------------

The program used in the ELBNF-GasTPC experiment for the Near Detector
is called:

*** GasTPCprocess ***

This is ran by:
GasTPCprocess input-card.in

or by:
GasTPCprocess card.1 card.2 card.3 etc
...
GasTPCprocess card.1 card.2 card.3 will then run the program. This will then output the file specified in outputFileName and should have three branches:
	NuHits			-	Flux
...
6.7 Other programs
----------------------
Other programs exist that only primaries are generated and ignores tracking via geant, this is GasTPCprimaries, and does not require inputs from the Geant4 Tracking Processor.
Also there is the GasTPCtracking which ignores the flux processor and reads in previous neutrino verticies from a previous run and only performs the tracking from geant.

7. Reading outputs/Analysis
--------------------------------
Some programs exist for reading the outputs, however you can make your own. GasTPCplot is a basic program to read file(s), from a batch run and loop over them and make some histograms.
