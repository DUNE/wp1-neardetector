...
void Geant4TrackingAlgorithm::runGunMac(){
    ...
    std::cout << "\n****Executing run script: " 
	      << "/control/execute " +  string(GasTPC) + "/src/simul/run.mac" <<std::endl; 
    UImanager->ApplyCommand("/control/execute " +  string(GasTPC) + "/src/simul/run.mac");
...
void Geant4TrackingAlgorithm::runVisMac(){
    ...
    std::cout << "\n****Executing visualisation script: " 
	      << "/control/execute " +  string(GasTPC) + "/src/simul/vis.mac" <<std::endl;
    UImanager->ApplyCommand("/control/execute " +  string(GasTPC) + "/src/simul/vis.mac");
...
void Geant4TrackingAlgorithm::initDataCards() {
  ...
  cards_->AddDataCardString("simGeomFileName", string(GasTPC) + "/geom_construction/workdir/geometry.gdml");
  ...
  cards_->AddDataCardDouble("simEnergyCut",100.0*CLHEP::keV);
  cards_->AddDataCardBool("trackPiZero",0);
  //physicsList Cuts
  cards_->AddDataCardDouble("physicsDefaultCut",0.01*CLHEP::cm);
  cards_->AddDataCardDouble("gammaCut",0.01*CLHEP::cm);
  cards_->AddDataCardDouble("electronCut",0.01*CLHEP::cm);
  cards_->AddDataCardDouble("positronCut",0.01*CLHEP::cm);
  cards_->AddDataCardDouble("neutronCut",0.01*CLHEP::cm);
  cards_->AddDataCardDouble("cutEM",0.01*CLHEP::cm);
