...
NeutrinoEvent* NuEventFileLoadAlgorithm::getPGEvent(int eventid) {
  ...
  _nuEvent->setInteractionType(-999);
  _nuEvent->setTopology(false, false, false, false, false, false, false, false, false, false);
  _nuEvent->setNuEnergy(-999);
  _nuEvent->setx(-999);
  _nuEvent->sety(-999);
  _nuEvent->setW(-999);
  _nuEvent->setQ2(-999);
  _nuEvent->setresid(-999);
  _nuEvent->setweight(1);
...
NeutrinoEvent* NuEventFileLoadAlgorithm::getGstEvent(int event) {

  NeutrinoEvent* _nuEvent = new NeutrinoEvent();
  inputTree_->GetEntry(event);
  
  _nuEvent->setEventID(event);
  //_nuEvent->setQELCC(qel);
  _nuEvent->setScatteringType(cc);
  _nuEvent->setInteractionType(neu);
  ...
  TLorentzVector vertex(vtxx,vtxy,vtxz,vtxt);
  //vertex *= CLHEP::m;
  _nuEvent->setPosition(vertex);
...
EventRecord* NuEventFileLoadAlgorithm::getNtpMCEventRecord(int eventid){
  ...
  //NtpMCEventRecord * newmcrec = mcrec;

  //return newmcrec;
  return mcrec->event;
}

NeutrinoEvent* NuEventFileLoadAlgorithm::getGHepEvent(int eventid) {
  ...
  // Store genie event record
  //_nuEvent->setGenieEventRecord(eventr);
  ...
  _nuEvent->setInteractionType(neutrino->Pdg());
  ...
  _nuEvent->setweight(wght);
  _nuEvent->setNEUTCode(neut_code);
  TLorentzVector vertex(vtx->X(),vtx->Y(),vtx->Z(),vtx->T());
  vertex *= CLHEP::m;
  _nuEvent->setPosition(vertex);
