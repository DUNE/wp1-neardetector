...
//#include "TpcPositionSmear.hh"
//#include "TpcDEDXSmear.hh"
...
int main(int argc, char ** argv) {
  ...
  //GasTPCTree->Branch("gmcrec","genie::NtpMCEventRecord", &fNtpMCEventRecord);
  ...
  //nuEventTree->Print();
  //genie::NtpMCEventRecord * grec;
  //if(!pg)
  //nuEventTree->GetBranch("gmcrec")->SetAddress(&grec);
  //nuEventTree->SetBranchAddress("gmcrec", &grec);
  //nuEventTree->Branch("gmcrec","genie::NtpMCEventRecord", &grec);
  ...
  // Position smearer
  //TpcPositionSmear *possmear   = new TpcPositionSmear();
  // Dedx smearer
  //TpcDEDXSmear     *dedxsmear  = new TpcDEDXSmear();
  // Momentum smearer
  //TpcMomSmear      *momsmearer = new TpcMomSmear();
  ...
  for(Int_t i=0; i<n; i++){
    ...
    //if(!pg){
    //if(grec){
    //genie::EventRecord *ev_rec = grec->event;
    //fNtpMCEventRecord->Fill(i, ev_rec);
      //}
    //}
    // Initialize
	...
	//if(npdg == 13)
	//  cout << (dedxrecon-dedxExpMuon)/dedxres << " , " << fdedx/dedxExpMuon << " , " <<  dedxres <<  endl;
	...
	if(value == tmpHit.getTrackID()){
	  ...
	  if(nhits == 1){
	    ...
	    //if(lecal != tmpHit.getEcalNumber())
	    //if(lecal != tmpHit.getDetector())
	    //necal++;
	    
	    //lecal = tmpHit.getEcalNumber();
	    //lecal =  tmpHit.getDetector();
	    
	    //temp = tmpHit.getEcalNumber(); 
	    //if(wecal != temp && wecal < 13)     
	    //wecal = wecal*10 + temp;
  ...
  //GasTPCTree->AddFriend("gmcrec",inputfile.c_str());
  // --------------------------------------------------------------------------------------
  delete rand;
  //delete possmear;
  //delete dedxsmear;
...
void ReadOutputTree(TTree *GasTPCTree){
  ...
  GasTPCTree->Branch("ReconPosition",                &ReconPosition,                "ReconPosition[NGeantTracks][3]/D");
  GasTPCTree->Branch("ReconLastPosition",            &ReconLastPosition,            "ReconLastPosition[NGeantTracks][3]/D");
