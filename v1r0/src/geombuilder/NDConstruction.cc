...
void NDConstruction::initializeTPC(double length, double radius, double density, std::string material){
  ...
  std::cout << "TPC mass = " << 3.14159*tpc_length_*tpc_radius_*tpc_radius_*tpc_density_ << std::endl;
}
...
void NDConstruction::construction(Materials * mats, bool ecal, bool magnet){
  ...
  TGeoBBox *worldbox = new TGeoBBox("WorldBox",3400,3400,3400);

  volume_ = new TGeoVolume("World",worldbox,wmedium);
  // TPC material
  ...
  volume_->AddNodeOverlap(Volume,0,new TGeoTranslation(0,0,0));
  //volume_ = new TGeoVolume("VesselVolume",tube_,medium);
  // TPC
  ...
  volume_->AddNodeOverlap(innerVolume_,1,new TGeoTranslation(0,0,0));
  //std::cout << "tpc_length_ = " << tpc_length_/(CLHEP::cm) << " , tpc_radius_ = " << tpc_radius_/(CLHEP::cm) << " , vessel_thickness_ = " << vessel_thickness_/(CLHEP::cm) << " , outradio_ = " << outradio_/(CLHEP::cm) << " , outlength_ = " << outlength_/(CLHEP::cm) << std::endl;
  //--------------- Anode and Cathode plates ------------------//
  ...
  volume_->AddNodeOverlap(anodeVolume_,2,new TGeoTranslation(0,0,tpc_length_/(2.0*(CLHEP::cm))));
  ...
  volume_->AddNodeOverlap(cathodeVolume_,3,new TGeoTranslation(0,0,-tpc_length_/(2.0*(CLHEP::cm))));
  // Ecal
  if(ecal){
    ...
    if(!ecal_buildlayers_){ // ecal without layers
      ...
      volume_->AddNodeOverlap(ez1volume,4,new TGeoTranslation(0,0,pos));
      ...
      pos = -pos;
      volume_->AddNodeOverlap(ez2volume,5,new TGeoTranslation(0,0,pos));
      ...
      volume_->AddNodeOverlap(ey1volume,6,new TGeoTranslation(0,pos,0));
      
      TGeoVolume *ey2volume = new TGeoVolume("EcalScintVolumeY2",motherBoxY,ecalmedium);
      pos = -pos;
      volume_->AddNodeOverlap(ey2volume,7,new TGeoTranslation(0,pos,0));
      ...
      pos = -pos;
      volume_->AddNodeOverlap(ex1volume,8,new TGeoTranslation(pos,0,0));
      
      TGeoVolume *ex2volume = new TGeoVolume("EcalScintVolumeX2",motherBoxX,ecalmedium);
      pos = -pos;
      volume_->AddNodeOverlap(ex2volume,9,new TGeoTranslation(pos,0,0));
    }
    else{ // Add layers
	...
	volume_->AddNodeOverlap(ez1volume,norder,new TGeoTranslation(0,0,pos));
	norder++;
	...
	volume_->AddNodeOverlap(ez2volume,norder,new TGeoTranslation(0,0,-pos));
	norder++;
	// Y
	y1 = z1;
	z1 = ecallength/(CLHEP::cm) + totalthickness/(CLHEP::cm);
	...
	volume_->AddNodeOverlap(ey1volume,norder,new TGeoTranslation(0,pos,0));
	norder++;
	...
	volume_->AddNodeOverlap(ey2volume,norder,new TGeoTranslation(0,-pos,0));
	norder++;
	...
	volume_->AddNodeOverlap(ex1volume,norder,new TGeoTranslation(pos,0,0));
	norder++;
	...
	volume_->AddNodeOverlap(ex2volume,norder,new TGeoTranslation(-pos,0,0));
	norder++;
	// Now the lead layer
	if(absthickness > 0.0){ // only add it if the thickness is greater than zero
	  ...
	  volume_->AddNodeOverlap(lz1volume,norder,new TGeoTranslation(0,0,pos));
	  norder++;
	  ...
	  volume_->AddNodeOverlap(lz2volume,norder,new TGeoTranslation(0,0,-pos));
	  norder++;
	  ...
	  volume_->AddNodeOverlap(ly1volume,norder,new TGeoTranslation(0,pos,0));
	  norder++;
	  ...
	  volume_->AddNodeOverlap(ly2volume,norder,new TGeoTranslation(0,-pos,0));
	  norder++;
	  ...
	  volume_->AddNodeOverlap(lx1volume,norder,new TGeoTranslation(pos,0,0));
	  norder++;
	  ...
	  volume_->AddNodeOverlap(lx2volume,norder,new TGeoTranslation(-pos,0,0));
	  norder++;
	}
    ...
    if(magnet){
      ...
      volume_->AddNodeOverlap(mz1volume,10,new TGeoTranslation(0,0,pos));
      ...
      pos = -pos;
      volume_->AddNodeOverlap(mz2volume,11,new TGeoTranslation(0,0,pos));
      ...
      volume_->AddNodeOverlap(my1volume,12,new TGeoTranslation(0,pos,0));
      
      TGeoVolume *my2volume = new TGeoVolume("MagnetVolumeY2",MmotherBoxY,magnetmedium);
      pos = -pos;
      volume_->AddNodeOverlap(my2volume,13,new TGeoTranslation(0,pos,0));
      ...
      pos = -pos;
      volume_->AddNodeOverlap(mx1volume,14,new TGeoTranslation(pos,0,0));
      
      TGeoVolume *mx2volume = new TGeoVolume("MagnetVolumeX2",MmotherBoxX,magnetmedium);
      pos = -pos;
      volume_->AddNodeOverlap(mx2volume,15,new TGeoTranslation(pos,0,0));
    }
