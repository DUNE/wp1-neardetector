...
void ScintillatorConstruction::initialize(){
  ...
  thickness_  = thickness_*(CLHEP::mm);
  gap_ = gap_*(CLHEP::mm);
  layersRequested_ = layers_;
}

void ScintillatorConstruction::construct(Materials * mats,TpcConstruction * tpc,TpcVesselConstruction * vessel){
  ...
  motherMaterial_ = (TGeoMixture*)mats->FindMaterial(tpc->getMatName().c_str());
 
  if( !motherMaterial_ ){
    motherMaterial_ = (TGeoMixture*)mats->FindMaterial("T2KArgonGas");
    motherMaterial_->SetDensity(tpc->getMaterial()->GetDensity()); //it should match the density of the tpc
    std::cout << "\n------------------------------------------------------------"
	      << "\nScintillatorConstruction::Construct MATERIAL NOT SET - USING T2KArgonGas"
	      << "\n------------------------------------------------------------" <<std::endl;
  }
  ...
  if(lengthDiff > 0)
    maxHeight = TMath::Sqrt( (vessel->getInnerSphereRadius()*vessel->getInnerSphereRadius())
  ...
  while( (maxHeight*maxHeight) <= ( (motherX*motherX)+(motherY*motherY) ) || ((motherX*motherX)+(motherY*motherY) > vessel->getInnerRadius()*vessel->getInnerRadius()) ){
    /*
  ...
  if( material_ ){
    ...
    std::cout << "\n------------------------------------------------------------"
	      << "\nScintillatorConstruction::Construct MATERIAL NOT SET - USING Scintillator"
	      << "\n------------------------------------------------------------" <<std::endl;
  }
  //medium
