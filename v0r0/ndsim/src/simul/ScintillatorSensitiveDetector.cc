...
ScintillatorSensitiveDetector::ScintillatorSensitiveDetector(const G4String& name,int layerID)
  :G4VSensitiveDetector(name){//,HitsCollection_(NULL),ecalNumber(layerID){
  HitsCollection_ = NULL;
  ecalNumber      = layerID;
  layerNumber     = -1;
  ...
  SDHit_.setEcalNumber(ecalNumber);
  if(ecalNumber > -1){
    std::stringstream ss;
    ss << ecalNumber;
...
ScintillatorSensitiveDetector::ScintillatorSensitiveDetector(const G4String& name, int ecal, int layerID)
  :G4VSensitiveDetector(name){//,HitsCollection_(NULL),ecalNumber(ecal),layerNumber(layerID){
  HitsCollection_ = NULL;
  ecalNumber      = ecal;
  layerNumber     = layerID;
  ...
  SDHit_.setLayerNumber(layerNumber);
  SDHit_.setEcalNumber(ecalNumber);
  if(ecalNumber > -1 && layerNumber > -1){
    std::stringstream ss;
    ss << layerNumber;
    G4String layerNoAsString = ss.str();

    std::stringstream ssecal;
    ssecal << ecalNumber;
    G4String ecalNoAsString = ssecal.str();
    
    SDname = name+"_"+ecalNoAsString+"_"+layerNoAsString;
    HCname = name + "HitsCollection"+"_"+ecalNoAsString+"_"+layerNoAsString;
  }
  else{
    SDname = name;
    HCname = name + "HitsCollection";
...
G4bool ScintillatorSensitiveDetector::ProcessHits(G4Step *aStep , G4TouchableHistory* touchHistory) {
  ...
  trackLeftVolume = 0;
  if( (trackNextVolumeName.compare(0,SDBaseName.size(),SDBaseName)!=0) )
    trackLeftVolume = 1;
  /* // george
  //set the muon
  ...
  */
  //std::cout << "Scint:: " << PDG_ << " , TrackID = " << TrackID_ << std::endl;
