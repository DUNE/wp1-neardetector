...
void EcalConstruction::initialize(){
...
void EcalConstruction::construct(Materials * mats,TpcVesselConstruction *vessel, std::string ecaldirection, bool buildlayers, int layernumber){
  ...
  innerRadius_ = vessel->getRadius()/(CLHEP::cm);
  radius_ = innerRadius_ +  gap/(CLHEP::cm);//vessel->getThickness()/(CLHEP::cm);
  length_ = vessel->getTubeLength()/(CLHEP::cm) /*+ vessel->getThickness()/(CLHEP::cm)*/ + gap/(CLHEP::cm); //(thickness_/4)/(CLHEP::cm);
...
void EcalConstruction::construct(Materials * mats,TpcVesselConstruction *vessel){
  ...
  innerRadius_ = vessel->getRadius()/(CLHEP::cm);
  radius_ = innerRadius_ + vessel->getThickness()/(CLHEP::cm);//thickness_;
  ...
  /*
  tube_ = new TGeoTube("EcalTube",innerRadius_/(CLHEP::cm),radius_/(CLHEP::cm),length_/(2.0*(CLHEP::cm)) );
  volume_ = new TGeoVolume("EcalVolume",tube_,medium_);
  volume_->GetMedium()->GetMaterial()->SetDensity(density_);
