...
public:
  ...
  const double getTrackLength(TVector3 first, TVector3 last);
  const double getTruthTransMomentum(HitCollection &track);
  const double getTruthTransMomentum(TLorentzVector momentum);
  ...
  const double getTrackSagitta(double trackLength,double transMom);
