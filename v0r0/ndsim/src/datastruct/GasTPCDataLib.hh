...
class NeutrinoEvent : public BaseEventRecord {
 public:
  ...
  void clearFssVector()  {fss_.resize(0); iss_.resize(0);}
  ...
  void setEventID(int event) {eventid_ = event;};
 ...
 private:
  ...
  int interactionType_, scatteringType_, resid_, eventid_, neutcode_, runid_;
  std::string nodeName_;

  //bool qelcc_;
  bool sea_, qel_, mec_, res_, dis_, coh_, dfr_, imd_, nuel_, charm_;

  double nuEnergy_, x_, y_, Q2_, W_, weight_;
...
class GeantTrackingTruth : public namedRecord {
 ...
 public:
  ...
  void setAllParticles(std::vector<GeantDaughterParticle> all) {allParticles = all;};
  ...
  void clearAllParticles()	{std::vector<GeantDaughterParticle>().swap(allParticles); allParticles.resize(0);};
...
protected:
  ...
  int nSteps_,PDG,trackID,parentID, charge_;
